 ...
 */
export const allAliases: Record<string,
  (action: {
    type: string
    payload: any
...
type AsyncThunkProps = keyof AsyncThunk<unknown, unknown, Record<string, unknown>>
...
const asyncThunkProperties = (() => {
  ...
  const exhaustiveList: ExhaustivePropList<typeof temp,
    AsyncThunk<unknown, unknown, Record<string, unknown>>> = temp
...
>(
  ...
  payloadCreator: AsyncThunkPayloadCreator<Returned, ThunkArg, any>,
  options?: AsyncThunkOptions<ThunkArg, any>
): ((payload: ThunkArg) => Action<TypePrefix> & { payload: ThunkArg }) &
  Pick<AsyncThunk<Returned, ThunkArg, any>, AsyncThunkProps> {
  ...
  const webextActionCreator = Object.assign(
    ...
    ) as Pick<AsyncThunk<Returned, ThunkArg, any>, AsyncThunkProps>
  )
  ...
  // the thunk correctly.
  allAliases[typePrefix] = (action: { type: string; payload: ThunkArg }) => baseThunkActionCreator(action.payload)
 ...
 * it completes".
 */
export type AsyncThunkFulfillmentType<T> = T extends Pick<// We don't really need the other two inferred values.
